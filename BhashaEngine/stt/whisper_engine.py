...
class OfflineSTTEngine:
    ...
    def transcribe(self, audio_path: str) -> str:
        ...
        try:
            ...
            # Fast fallback: pass file path directly (Whisper pipeline can handle many formats IF ffmpeg is installed)
            try:
                result = self.pipe(audio_path)
                if not result or "text" not in result:
                    raise ValueError("Incomplete transcription result.")
                return result["text"].strip()
            except Exception as e2:
                # Check if likely due to missing ffmpeg
                error_str = str(e2).lower()
                if "ffmpeg" in error_str or "avconv" in error_str:
                    raise Exception("Audio decoding failed. Please install FFmpeg (https://ffmpeg.org/download.html) and add to PATH.")
